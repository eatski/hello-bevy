...
/// 型情報を活用するHeroコンバーター
pub struct TypedHeroConverter;

impl TypedNodeConverter<TeamSide> for TypedHeroConverter {
    fn can_convert(&self, token: &StructuredTokenInput, expected_type: &Type) -> bool {
        matches!(token, StructuredTokenInput::Hero) && 
        matches!(expected_type, Type::TeamSide)
    }
    ...
    fn convert(&self, 
               _typed_ast: &TypedAst, 
               _registry: &dyn TypedConverterRegistry) -> Result<Box<ActionSystemNode<TeamSide>>, String> {
        Ok(Box::new(TeamSideConstantNode::new(TeamSide::Player)))
    }
}
