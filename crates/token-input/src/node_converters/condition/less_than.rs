...
use action_system::*;
use action_system::nodes::condition::{LessThanNode, CharacterHpVsValueLessThanNode, ValueVsCharacterHpLessThanNode};
...
/// 型情報を活用するLessThanコンバーター
pub struct TypedLessThanConverter;

impl TypedLessThanConverter {
    /// Numeric型の実際の型を推論
...
impl TypedNodeConverter<bool> for TypedLessThanConverter {
    fn can_convert(&self, token: &StructuredTokenInput, expected_type: &Type) -> bool {
        matches!(token, StructuredTokenInput::LessThan { .. }) && 
    ...
    fn convert(&self, 
        ...
        if let StructuredTokenInput::LessThan { .. } = &typed_ast.token {
            // 子要素の型情報を確認
            let left_ast = typed_ast.children.get("left")
                .ok_or_else(|| "LessThan requires a left argument".to_string())?;
            let right_ast = typed_ast.children.get("right")
                .ok_or_else(|| "LessThan requires a right argument".to_string())?;
            ...
            match (&actual_left_type, &actual_right_type) {
                (Type::I32, Type::I32) => {
                    ...
                    Ok(Box::new(LessThanNode::<i32>::new(left_node, right_node)))
                }
                (Type::CharacterHP, Type::CharacterHP) => {
                    ...
                    Ok(Box::new(LessThanNode::<CharacterHP>::new(left_node, right_node)))
                }
                (Type::CharacterHP, Type::I32) => {
                    let left_node = convert_child::<CharacterHP>(registry, typed_ast, "left")?;
                    let right_node = convert_child::<i32>(registry, typed_ast, "right")?;
                    Ok(Box::new(CharacterHpVsValueLessThanNode::new(left_node, right_node)))
                }
                (Type::I32, Type::CharacterHP) => {
                    let left_node = convert_child::<i32>(registry, typed_ast, "left")?;
                    let right_node = convert_child::<CharacterHP>(registry, typed_ast, "right")?;
                    Ok(Box::new(ValueVsCharacterHpLessThanNode::new(left_node, right_node)))
                }
                _ => Err(format!("LessThan not supported for types {:?} and {:?}", 
                               actual_left_type, actual_right_type))
            }
        } else {
            Err("Expected LessThan token".to_string())
        }
    }
}